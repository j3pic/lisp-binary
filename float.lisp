...
(defun nanp (decoded-value)
  (or #-float-signalling-nan
      ...
      (/= (cffi:foreign-funcall "isnan" :double (coerce decoded-value 'double-float) :int) 0)))
...
use this function."
  ...
  (and (not (nanp decoded-value))
       #+float-infinity
       (values (/= (cffi:foreign-funcall "isinf" :double (coerce decoded-value 'double-float) :int) 0)
	       (> decoded-value 0))))
